 ...
 * No config, no held-food preview overlay, no health-bar overlay —
 * just the current saturation (on the hunger bar) and current
 * exhaustion (underneath it).
 */
public class HUDOverlayHandler {
	...
	public void onRenderFood(GuiGraphics context, Player player, int top, int right) {
		...
		if (!saturationRenderEvent.isCanceled) {
			this.drawSaturationOverlay(saturationRenderEvent, mc, mc.gui.getGuiTicks());
		}
	}

	public void drawSaturationOverlay(GuiGraphics context, float saturationLevel, Minecraft mc, int right, int top, int guiTicks) {
	...
	public void drawExhaustionOverlay(GuiGraphics context, float exhaustion, int right, int top) {
	...
	private static class OffsetsCache {
		...
		protected void generate(int guiTicks, Player player) {
			...
			this.random.setSeed(guiTicks * 312871L);
